...
void Controller::countRevolution() {
  ...
  revolutions++;
  totalRevolutions++; //Total revolutions is only modified and accessed within this function so 
...
void Controller::pulseOn() {
 // if (!INJisDisabled) { Removed because when pulseOn() is called, shouldn't the injector always inject fuel, otherwise there is possibility
 //of injector not working every second revolution
